...
//	printf("before vid_begin\r\n");

	vid_begin();
//	printf("before lcd_begin\r\n");
	lcd_begin();
	printf("after lcd_begin\r\n");
	for(y = 0; y < 144; ++y) {
	...
	printf("end vid_end\r\n");
	restore_pal(&bk);
	printf("end restore_pal\r\n");
}

static int menu_getevent(int *st) {
	event_t ev;
	int polled = 0;
//	printf("memnu get event\r\n");
	while (1) {
//	printf("memnu get event while1\r\n");
		if(!ev_getevent(&ev)) {
//			printf("memnu get event not getevent\r\n");
			if(polled) break;
			ev_poll(1);
			polled = 1;
			continue;
		}
//		printf("memnu get event press\r\n");
		if (ev.type != EV_PRESS && ev.type != EV_RELEASE)
			continue;
		*st = (ev.type != EV_RELEASE);
		return ev.code;
	}
//	printf("memnu get event return\r\n");
	return 0;
}
